...
class EditScreen extends ConsumerStatefulWidget {
  ...
  const EditScreen({super.key, required this.observation});
...
class _EditScreenState extends ConsumerState<EditScreen> {
  ...
  void initState() {
    ...
    _latController = TextEditingController(text: obs.latitude.toStringAsFixed(6));
    _lngController = TextEditingController(text: obs.longitude.toStringAsFixed(6));
  ...
  void _parseCatatanHabitat(String catatan) {
    try {
      if (catatan.contains('|')) {
        final parts = catatan.split('|').map((e) => e.trim()).toList();
        for (var p in parts) {
          if (p.startsWith('Status:')) {
            String val = p.replaceFirst('Status:', '').trim();
            if (_listKesehatan.contains(val)) _statusKesehatan = val;
            else { _statusKesehatan = 'Lainnya'; _kesehatanCustomController.text = val; }
          } else if (p.startsWith('Habitat:')) {
            String val = p.replaceFirst('Habitat:', '').trim();
            if (_listHabitat.contains(val)) _tipeHabitat = val;
            else { _tipeHabitat = 'Lainnya'; _habitatCustomController.text = val; }
          } else if (p.startsWith('Posisi:')) {
            String val = p.replaceFirst('Posisi:', '').trim();
            if (_listPosisi.contains(val)) _posisiSatwa = val;
            else { _posisiSatwa = 'Lainnya'; _posisiCustomController.text = val; }
          } else if (p.startsWith('Veg:')) {
            _vegetasiController.text = p.replaceFirst('Veg:', '').trim();
          } else if (p.startsWith('Note:')) {
            _catatanController.text = p.replaceFirst('Note:', '').trim();
          }
        }
      } else {
    ...
    if (widget.observation.aktivitasTermati != null) {
      if (_listAktivitas.contains(widget.observation.aktivitasTermati)) {
        _aktivitas = widget.observation.aktivitasTermati!;
      } else {
        _aktivitas = 'Lainnya';
        _aktivitasCustomController.text = widget.observation.aktivitasTermati!;
      }
    }
  }
